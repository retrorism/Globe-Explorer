...
colladaLoader.load( 'data/mannetje1.dae', function ( collada ) {
	figureModels.push(collada.scene);
});
colladaLoader.load( 'data/mannetje2.dae', function( collada ) {
	figureModels.push(collada.scene);
});
//console.log( daedae );
...
DAT.Globe = function(container, opts) {
	...
	function addFigure( lat, lng, subgeo, name) {
		var no = Math.floor(Math.random() * 2) + 1;
		var texture = THREE.ImageUtils.loadTexture( imgDir + "mannetje" + no + "-texture.png");
		var toClone = figureModels[no-1];
		figure = toClone.children[0].children[0].clone();
		var material = new THREE.MeshBasicMaterial( { map: texture } );
		figure.material = material;
		figure.material.needsUpdate = true;
		console.log( 'this is the figure' );
		console.log( figure );
		var phi = (90 - lat) * Math.PI / 180;
		var theta = (180 - lng) * Math.PI / 180;
		figure.lat = lat;
		figure.lng = lng;
		figure.position.x = 200 * Math.sin(phi) * Math.cos(theta);
		figure.position.y = 200 * Math.cos(phi);
		figure.position.z = 200 * Math.sin(phi) * Math.sin(theta);
		figure.lookAt(earth.position);
		figure.rotateX(Math.PI / 4 );
		figure.rotateZ(Math.PI );
		figure.scale.x = 3;
		figure.scale.y = 1;
		figure.scale.z = 3;
		//subgeo.merge(figure.geometry, figure.matrix);
		//figure.geometry.computeFaceNormals();
		targetList.push( figure );
		scene.add( figure );
	}

	function addPoint(lat, lng, size, color, subgeo, name) {
		addFigure( lat, lng, subgeo, name );
		// var phi = (90 - lat) * Math.PI / 180;
		// var theta = (180 - lng) * Math.PI / 180;
		// point.name= name;
	...
	function onMouseDown(event) {
		...
		picker.x = ( event.clientX / window.innerWidth ) * 2 - 1;
		picker.y = - ( event.clientY / window.innerHeight ) * 2 + 1;

		mouseOnDown.x = - event.clientX;
		mouseOnDown.y = event.clientY;

		raycaster.setFromCamera( picker, camera );

		var intersects = raycaster.intersectObjects( targetList );

		if ( intersects.length > 0 ) {
			console.log( intersects[ 0 ].object );
			var targetx = parseInt(intersects[0].object.lat);
		    var targety = parseInt(intersects[0].object.lng);
